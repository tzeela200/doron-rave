...
function insertRecord_(sheetName, idColumnName, data) {
  ...
  try {
    ...
    // כתיבה עם פורמט מפורש לשורה (במקום appendRow), כדי שטקסט יישאר טקסט ותאריך יישאר תאריך
    var row = objectToRow_(record, headers);
    var range = sheet.getRange(sheet.getLastRow() + 1, 1, 1, row.length);
    range.setNumberFormats([getColumnFormats_(headers)]);
    range.setValues([row]);
...
function updateRecord_(sheetName, idColumnName, idValue, updates) {
  ...
  lock.waitLock(10000);
  try {
    // כתיבה לעולם לא נשענת על זיכרון: קוראים את השורה מחדש בתוך ה-Lock, כדי לא לדרוס שינוי של משתמש אחר
    invalidateSheetMemo_(sheetName);
    ...
    var range = sheet.getRange(found.rowIndex, 1, 1, row.length);
    range.setNumberFormats([getColumnFormats_(headers)]);
    range.setValues([row]);
    invalidateSheetMemo_(sheetName);
