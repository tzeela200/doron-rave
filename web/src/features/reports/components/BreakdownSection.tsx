...
export function BreakdownSection({ eventId, total }: { eventId: string; total: number }) {
  ...
  return (
    <Section id="reports" title="דוחות ופירוקים" icon={PieChart} iconTone="neutral" meta={<>סה״כ הוצאות מוסכמות <Money value={total} /></>}>
...
export function VendorsContactsSection({ vendors }: { vendors: { id: string; name: string; phone: string; expenseNames: string[] }[] }) {
  ...
  return (
    <Section id="vendors" title="ספקים ואנשי קשר" count={vendors.length} icon={Truck} iconTone="warning">
      <Card padded={false}>
